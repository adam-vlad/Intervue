...
public class CvEndpointsTests : IClassFixture<IntervueWebApplicationFactory>
{
    ...
    [Fact]
    public async Task GetAllCvProfiles_ReturnsOkWithList()
    {
        // Act
    ...
    [Fact]
    public async Task GetAllCvProfiles_AfterUpload_ContainsUploadedProfile()
    {
        // Arrange — upload a valid PDF first
        var pdfContent = CreateMinimalPdf();
        using var uploadContent = new MultipartFormDataContent();
    ...
    // ── GET /api/v1/cv/{id}/interviews ───────────────────────────────

    [Fact]
    public async Task GetInterviewsByCv_WithNonExistentCvId_Returns404NotFound()
    {
        // Act
        var response = await _client.GetAsync($"/api/v1/cv/{Guid.NewGuid()}/interviews");
