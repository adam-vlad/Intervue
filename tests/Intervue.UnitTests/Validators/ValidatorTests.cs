...
public class ValidatorTests
{
    ...
    // ── GetInterviewsByCvValidator ──────────────────────────────────

    private readonly GetInterviewsByCvValidator _getInterviewsByCvValidator = new();

    [Fact]
    public void GetInterviewsByCvValidator_WithValidId_PassesValidation()
    {
        var query = new GetInterviewsByCvQuery(Guid.NewGuid());
        var result = _getInterviewsByCvValidator.TestValidate(query);
        result.ShouldNotHaveAnyValidationErrors();
    }

    [Fact]
    public void GetInterviewsByCvValidator_WithEmptyId_FailsValidation()
    {
        var query = new GetInterviewsByCvQuery(Guid.Empty);
        var result = _getInterviewsByCvValidator.TestValidate(query);
        result.ShouldHaveValidationErrorFor(x => x.CvProfileId)
              .WithErrorMessage("CvProfileId is required.");
    }
