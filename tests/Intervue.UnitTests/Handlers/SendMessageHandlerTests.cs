...
public class SendMessageHandlerTests
{
    ...
    public async Task Handle_WithValidMessage_ReturnsFollowUpQuestion()
    {
        ...
        // Assert — handler returns the full updated interview (candidate + AI follow-up appended)
        result.IsSuccess.Should().BeTrue();
        result.Value.Should().NotBeNull();
        result.Value!.Messages.Should().HaveCount(3); // initial AI question + candidate answer + AI follow-up
        result.Value.Messages[^1].Role.Should().Be(MessageRole.Interviewer);
        result.Value.Messages[^1].Content.Should().Contain("SOLID");
