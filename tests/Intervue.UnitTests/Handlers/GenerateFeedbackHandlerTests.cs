...
public class GenerateFeedbackHandlerTests
{
    ...
    public async Task Handle_WithValidInterview_ReturnsFeedbackReport()
    {
        ...
        _cvProfileRepository.Setup(x => x.GetByIdAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task Handle_WhenCvProfileLoaded_SystemPromptContainsCalibrationContext()
    {
        // Arrange
        var interviewId = Guid.NewGuid();
        var command = new GenerateFeedbackCommand(interviewId);
        var interview = CreateInterviewWithMessages(3);
        var cvProfile = CvProfile.Create("John\nSenior Dev", new HashedPersonalData("hash"));
        cvProfile.SetParsedData(DifficultyLevel.Senior, "BSc CS",
            new[] { Technology.Create("C#", 6) }.ToList(),
            new List<Experience>(), new List<Project>());
        ...
        // Assert
        capturedMessages.Should().NotBeNull();
        capturedMessages![0].Content.Should().Contain("Senior");
        capturedMessages[1].Content.Should().Contain("Level: Senior");
        capturedMessages[1].Content.Should().Contain("C#");
    }
