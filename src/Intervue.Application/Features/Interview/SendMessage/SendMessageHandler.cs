...
/// 5. Returns the full updated InterviewDto so the client has the complete conversation state
/// </summary>
public class SendMessageHandler : IRequestHandler<SendMessageCommand, Result<InterviewDto>>
{
    ...
    public async Task<Result<InterviewDto>> Handle(SendMessageCommand request, CancellationToken cancellationToken)
    {
        ...
        // Step 7: Return the full updated InterviewDto so the client always has the complete state
