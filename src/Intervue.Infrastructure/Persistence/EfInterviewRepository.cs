...
public class EfInterviewRepository : IInterviewRepository
{
    ...
    public async Task<IReadOnlyList<Interview>> GetByCvProfileIdAsync(Guid cvProfileId, CancellationToken cancellationToken = default)
    {
        return await _db.Interviews
            .AsNoTracking()
            .Where(i => i.CvProfileId == cvProfileId)
            .Include(i => i.Messages.OrderBy(m => m.SentAt))
            .Include(i => i.FeedbackReport)
