...
public class CvController : ControllerBase
{
    ...
    /// <summary>
    /// Get all CV profiles as lightweight summaries for the dashboard.
    /// </summary>
    [HttpGet]
    public async Task<IActionResult> GetAllCvProfiles(CancellationToken cancellationToken)
    {
    ...
    /// <summary>
    /// Get all interviews for a specific CV profile.
    /// </summary>
    [HttpGet("{id:guid}/interviews")]
    public async Task<IActionResult> GetInterviewsByCv(Guid id, CancellationToken cancellationToken)
    {
        var query = new GetInterviewsByCvQuery(id);
        var result = await _mediator.Send(query, cancellationToken);
        return result.ToActionResult();
    }
}
